...
{
    // q: my_malloc_handler是干嘛的，在哪里初始化？
...
// q: 作用？  typedef malloc_alloc 指定 alloc 实际执行器
...
/*
 * 通过设置静态变量 char* start, char* end，控制内存池
 * 内存分配/释放 allocate、deallocate，均为静态成员函数，单线程情况下，操作内存池无需锁。
 */
...
# endif
  static size_t ROUND_UP(size_t bytes) {
        ...
        // 注：ROUND_UP 计算逻辑等价于 (bytes + 7) / 8 * 8
__PRIVATE:
  union obj {
        union obj * free_list_link;
        char client_data[1];    /* The client sees this.        */     // client_data[1]标识作用，表示可以转化为字节数组首地址
  };
private:
# ifdef __SUNPRO_CC
...
# endif
  static  size_t FREELIST_INDEX(size_t bytes) {
        return (((bytes) + __ALIGN-1)/__ALIGN - 1);         // 注：[0-15]
  }
...
#       endif
    ...
    *my_free_list = result -> free_list_link;   // q：这一句的作用是啥？  a: 取出一块内存，并将free_list指向下一个空闲块
    return (result);
  };
...
__default_alloc_template<threads, inst>::chunk_alloc(size_t size, int& nobjs)
{
    ...
    if (bytes_left >= total_bytes) {
        ...
        start_free = (char *)malloc(bytes_to_get);       // 先从堆上申请内存
        if (0 == start_free) {                      // 如果未申请成功，则从free_list中查找空闲块
            int i;
            ...
            for (i = size; i <= __MAX_BYTES; i += __ALIGN) {
                ...
                p = *my_free_list;
                if (0 != p) {
                    *my_free_list = p -> free_list_link;     // 取出一块,next指向下一块
                    start_free = (char *)p;
                                                    ...
                                                    size_t new_sz)
{
    ...
    result = allocate(new_sz);                            // q: new_sz分配是必须的?   a: new_size<old_size 也必须重新分配
                                                          // a: 否则返回的内存块大小为old_size

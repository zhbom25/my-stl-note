...
/// slist: 1. 单向链表，存储head标识节点，不进行构造初始化；存储空间不连续，->next实现++操作
///        2. head节点类型为__slist_node_base，存储next指针，没有存T元素，这样allocate内存时，head节点节省了T的部分
///        3. 其他数据节点类型为__slist_node*，存储T元素
///        4. iterator类型为__slist_iterator<T,T&,T*>，通过__slist_node_base*进行操作，head节点不会生成迭代器
...
private:
  list_node_base head;           // 不存内容，链表前置节点
...
private:
  ...
  list_node_base* erase_after(list_node_base* pos) {
    ...
    // 删除 (before_first,last_node)区间内节点，即[first,last)区间内节点
...
public:
  ...
  void insert_after(iterator pos,
    ...
    // insert系列：在指定节点前，插入新节点；实现：查找到指定节点的前一个节点，执行insert_after
...
public:
    // erase_after删除指定节点的后一个节点，即 pos->next；返回 pos->next->next
